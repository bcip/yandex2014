...
class TaskE {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		final int[] cLength = new int[count];
		final String[] cNext = new String[count];
		final int[] aLength = new int[count];
		final String[] aNext = new String[count];
			...
			for (int j = 0; j < s.length(); j++) {
				if (s.charAt(j) != 'a') {
					if (first) {
						first = false;
						cLength[i] = j;
						cNext[i] = s.substring(j);
					} else {
						second = false;
						aLength[i] = j;
						aNext[i] = s.substring(j);
						break;
					}
				}
			}
		...
		int[] cOrder = ArrayUtils.createOrder(count);
		ArrayUtils.sort(cOrder, new IntComparator() {
			@Override
			public int compare(int first, int second) {
				if (cLength[first] != cLength[second])
					return cLength[first] - cLength[second];
				return cNext[second].compareTo(cNext[first]);
			}
		});
		int[] aOrder = ArrayUtils.createOrder(count);
		ArrayUtils.sort(aOrder, new IntComparator() {
			@Override
			public int compare(int first, int second) {
				if (aLength[first] != aLength[second])
					return aLength[first] - aLength[second];
				return aNext[second].compareTo(aNext[first]);
			}
		});
		int j = 0;
		long answer = 0;
		for (int i : aOrder) {
			while (j < count && (cLength[cOrder[j]] < aLength[i] || cLength[cOrder[j]] == aLength[i] && cNext[cOrder[j]].compareTo(aNext[i]) > 0))
				j++;
			answer += j - 1;
			if (cNext[i].isEmpty())
				answer++;
