...
class TaskE {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		final String[] c = new String[count];
		...
		for (int i = 0; i < count; i++) {
			...
			for (int j = 0; j < s.length(); j++) {
				if (s.charAt(j) != 'a') {
					first = false;
					a[i] = s.substring(0, j) + 'a' + s.substring(j + 1);
					break;
				}
			}
			if (first)
		...
		ArrayUtils.sort(cOrder, new IntComparator() {
			...
			public int compare(int first, int second) {
				return c[second].compareTo(c[first]);
			}
		});
		...
		ArrayUtils.sort(aOrder, new IntComparator() {
			@Override
			public int compare(int first, int second) {
				return a[second].compareTo(a[first]);
			}
		});
		int j = 0;
		long answer = 0;
		for (int i : aOrder) {
			while (j < count && c[cOrder[j]].compareTo(a[i]) > 0)
				j++;
			answer += j - 1;
			if (c[i].equals(a[i]))
				answer++;
